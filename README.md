...
2.1. via `htpasswd` shell command
```
htpasswd -c ~/dockerui/.htpasswd [username]
```
2.2. or via one of these websites [htpasswd generators](http://goo.gl/yLfKmV) (opens google search)
...
2. Run dockerui with additional parameter `-v ~/dockerui/ssl:/etc/nginx/ssl`